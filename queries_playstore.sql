...
SELECT app_name, price, reviews FROM analytics WHERE price BETWEEN 0.10 AND 1.00 ORDER BY reviews desc LIMIT
 10;
...
--have been installed at least 100,000 times. Display the name of the app along with the number of reviews, 
--the min_installs, and the proportion.
SELECT app_name, reviews, min_installs FROM analytics WHERE min_installs >= 100000 ORDER BY min_installs des
c LIMIT 1;
...
SELECT app_name, rating FROM analytics WHERE min_installs >= 500000 ORDER BY category, rating desc;

FS2. --Find all the apps that have a name similar to “facebook”
SELECT app_name FROM analytics WHERE app_name SIMILAR TO 'Facebook' 

FS3. --Find all the apps that have more than 1 genre
SELECT app_name FROM analytics WHERE genres[2] != '';
...
SELECT app_name FROM analytics WHERE 'Education' = ANY(genres) AND array_length(genres, 1) > 1;